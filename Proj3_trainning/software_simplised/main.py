...
from utils.mlflow_paths import local_artifact_path
...
def train(model, train_loader, val_loader, criterion, optimizer, args, artifact_root):
    ...
    for epoch in range(args.num_epochs):
        ...
        torch.save(model.state_dict(), artifact_root / f"model_epoch{epoch}.pth")

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            # save the new best model to MLflow artifact with 3 decimal places of validation loss in the file name
            torch.save(model.state_dict(), artifact_root / "model_best_val_loss.pth")
        ...
        if best_val_p10_acc < val_metrics['val_p_acc']['val_p10_acc']:
            ...
            torch.save(model.state_dict(), artifact_root / "model_best_p10_acc.pth")

        if best_val_p5_acc < val_metrics['val_p_acc']['val_p5_acc']:
            best_val_p5_acc = val_metrics['val_p_acc']['val_p5_acc']
            torch.save(model.state_dict(), artifact_root / "model_best_p5_acc.pth")
...
def main(args):
    ...
    # AutoDL 适配：run.sh 会为每次运行指定独立的清单、元数据和缓存目录。
    # 使用 getattr 保留 ESDA 的默认行为，直接执行 main.py 时仍可使用原目录。
    args.data_list_dir = getattr(args, "data_list_dir", "./dataset")
    args.metadata_root = getattr(args, "metadata_root", "./metadata")
    args.cache_root = getattr(args, "cache_root", "./cached_dataset")
    ...
    # set_experiment 会在全新的 MLflow 文件仓库中自动创建命名实验。
    # 不能直接假设 experiment ID 0 存在，否则首次运行会报找不到实验。
    ...
    with mlflow.start_run(
        ...
        # 保存检查点前，把 MLflow 的 file:// artifact URI 转为普通本地 Path。
        # 训练结束后，run.sh 会把整个 MLflow 目录复制到持久盘。
        artifact_root = local_artifact_path(mlflow.get_artifact_uri())
        ...
        with (artifact_root / "args.json").open("w", encoding="utf-8") as f:
        ...
        # AutoDL 适配：每次运行的切片元数据单独保存，不写入源码目录。
        # 这里保存的是切片索引，不是模型检查点。
        train_metadata_path = os.path.join(
            args.metadata_root,
            f"3et_train_tl_{args.train_length}_ts{args.train_stride}_ch{args.n_time_bins}{cached_item}",
        )
        val_metadata_path = os.path.join(
            args.metadata_root,
            f"3et_val_vl_{args.val_length}_vs{args.val_stride}_ch{args.n_time_bins}{cached_item}",
        )
        ...
        train_cached_path = os.path.join(
            args.cache_root,
            f"train_tl_{args.train_length}_ts{args.train_stride}_ch{args.n_time_bins}{cached_item}",
        )
        val_cached_path = os.path.join(
            args.cache_root,
            f"val_vl_{args.val_length}_vs{args.val_stride}_ch{args.n_time_bins}{cached_item}",
        )
        ...
        # 部分 float32 配置没有 augment_path，因此把它作为可选参数处理。
        # argparse.Namespace 不能使用 `"augment_path" in args` 判断成员。
        augment_path = getattr(args, "augment_path", None)
        ...
        torch.save(
            model.state_dict(),
            artifact_root / f"model_last_epoch{args.num_epochs}.pth",
        )
...
if __name__ == "__main__":
    ...
    # 这三个目录由 run.sh 传入，使生成的元数据/缓存不污染 software/，
    # 并让每次运行的产物可以独立持久化。
